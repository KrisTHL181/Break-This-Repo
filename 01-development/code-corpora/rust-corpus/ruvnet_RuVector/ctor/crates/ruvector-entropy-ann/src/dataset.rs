...
///
/// The seed is fixed so datasets are deterministic across runs.
...
/// Cluster-structured dataset: `num_clusters` Gaussian blobs.
///
/// Simulates the typical agent-memory workload where embeddings cluster by topic.
/// Queries near cluster centroids are "easy" (low entropy); queries midway between
/// clusters are "hard" (high entropy).
pub fn clustered_vectors(
    ...
    // Assign each vector to a cluster + noise
    (0..n)
        .map(|i| {
...
mod tests {
    ...
    fn ground_truth_returns_k() {
        ...
        assert_eq!(gt.len(), 10);
        // The query itself should be the nearest to itself
        assert_eq!(gt[0], 0);
    }
}

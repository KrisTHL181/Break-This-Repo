...
impl ExactCache {
    ...
    fn lookup(&self, query: &[f32], k: usize) -> Option<Vec<Hit>> {
        let h = hash_query(query);
        for e in &self.entries {
            if e.hash == h && e.query == query && e.results.len() >= k {
                return Some(e.results[..k].to_vec());
            }
        }
        None
    }
...
impl CachedAnn for ExactCache {
    fn search(&mut self, query: &[f32], k: usize) -> (Vec<Hit>, CacheDecision) {
        if let Some(cached) = self.lookup(query, k) {
            self.stats.hits += 1;
...
mod tests {
    ...
    fn zero_capacity_disables_storage() {
        let mut c = ExactCache::new(tiny_corpus(), 0);
        let q = vec![1.5f32, 0.0];

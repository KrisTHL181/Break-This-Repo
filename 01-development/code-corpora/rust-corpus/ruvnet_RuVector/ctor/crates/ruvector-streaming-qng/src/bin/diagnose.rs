...
fn main() {
    ...
    for c in 0..n_clusters {
        for i in 0..n_per_cluster {
            ...
            for d in 1..dims {
                ...
                v.push(
                    centroid_d + (seed_val * 17.0 + d as f32 * std::f32::consts::PI).sin() * std,
                );
            }
            vecs.push(v);
        }
    ...
    println!(
        "Ground truth top-5 ids: {:?}",
        gt.iter().take(5).map(|h| h.id).collect::<Vec<_>>()
    );
    println!(
        "All top-10 from cluster 0? {}",
    ...
    println!(
        "True sq_l2 to cluster-0 vector: {:.4}",
        sq_l2(&query, &vecs[0])
    );
    println!(
        "True sq_l2 to cluster-1 vector: {:.4}",
        sq_l2(&query, &vecs[200])
    );

    // Compare all subspace-0 centroid dim0 values
    println!("\nAll subspace-0 centroid dim0 values:");
    let mut dim0_vals: Vec<(usize, f32)> = cb.centroids[0]
        .iter()
        .enumerate()
        .map(|(i, c)| (i, c[0]))
        .collect();
    ...
    println!(
        "Cluster-1 subspace-0 centroid: centroid[{}].dim0 = {:.4}",

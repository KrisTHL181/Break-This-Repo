...
impl Codebook {
    ...
    pub fn train(samples: &[Vec<f32>], m: usize, k: usize, seed: u64) -> Self {
        ...
        assert!(m > 0, "number of subspaces must be greater than zero");
        assert!(k > 0, "centroids per subspace must be greater than zero");
        assert!(k <= u8::MAX as usize + 1, "centroid count must fit in u8");
        let d = samples[0].len();
        assert!(d > 0, "vectors must have at least one dimension");
        assert!(
            samples.iter().all(|sample| sample.len() == d),
            "all samples must have the same dimension"
        );
        assert!(
            samples.iter().flatten().all(|value| value.is_finite()),
            "all sample coordinates must be finite"
        );
        ...
        for sub in 0..m {
            let start = sub * ds;
            let end = start + ds;
            let slices: Vec<Vec<f32>> = samples.iter().map(|v| v[start..end].to_vec()).collect();
            ...
            for _iter in 0..TRAIN_ITERS {
                ...
                for c in 0..k_eff {
                    if counts[c] > 0 {
                        centers[c] = sums[c].iter().map(|&s| s / counts[c] as f32).collect();
                    }
                }
            }
    ...
    pub fn update_one_pass(&mut self, reservoir: &[Vec<f32>]) {
        ...
        for sub in 0..self.m {
            ...
            let slices: Vec<Vec<f32>> = reservoir.iter().map(|v| v[start..end].to_vec()).collect();

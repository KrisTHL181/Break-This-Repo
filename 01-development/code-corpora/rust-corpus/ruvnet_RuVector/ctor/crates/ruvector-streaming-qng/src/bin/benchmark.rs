...
use ruvector_streaming_qng::{
    full_precision::FullPrecision, static_pq::StaticPq, stream_pq::StreamPq, AnnVariant, Hit,
};
...
impl BenchCfg {
    ...
    fn phase_a_total(&self) -> usize {
        self.clusters * self.n_per_a
    }
    fn phase_b_total(&self) -> usize {
        self.clusters * self.n_per_b
    }
    fn queries_total(&self) -> usize {
        self.clusters * self.queries_per_cluster
    }
    fn phase_b_start(&self) -> usize {
        self.phase_a_total()
    }
...
fn centroid(c: usize, dims: usize, n_clusters: usize) -> Vec<f32> {
    (0..dims)
        .map(|d| {
            if d == 0 {
                c as f32 * CLUSTER_SPACING
            } else {
                ((c * 7 + d * 3) % n_clusters) as f32 * 0.3
            }
        })
        .collect()
}
...
fn gen_block(
    ...
    for c in 0..clusters {
        ...
        for _ in 0..n_per_cluster {
            let v: Vec<f32> = cent
                .iter()
                .map(|&x| x + shift + normal.sample(&mut rng))
                .collect();
            vecs.push(v);
        }
    }
...
fn cluster_prec(hits: &[Hit], b_start: usize, b_end: usize, k: usize) -> f32 {
    let correct = hits
        .iter()
        .take(k)
        .filter(|h| h.id >= b_start && h.id < b_end)
        .count();
...
fn main() {
    ...
    println!();
    println!(
        "  Clusters:  {C}  dims={d}  shift={SHIFT:.1}  std={CLUSTER_STD}",
        C = cfg.clusters,
        d = cfg.dims
    );
    println!("  Phase A:   {n_a} vectors ({} per cluster)", cfg.n_per_a);
    println!(
        "  Phase B:   {n_b} vectors ({} per cluster, {:.0}× Phase A for reservoir domination)",
        cfg.n_per_b,
        cfg.n_per_b as f64 / cfg.n_per_a as f64
    );
    println!(
        "  Queries:   {q_total} Phase-B ({} per cluster)  k={k}",
        cfg.queries_per_cluster,
        k = cfg.k
    );
    println!();
    ...
    let qa_ranges: Vec<(usize, usize)> = (0..cfg.clusters)
        .flat_map(|c| {
            let r = cfg.phase_a_range(c);
            (0..cfg.queries_per_cluster).map(move |_| r)
        })
        .collect();

    // cluster ranges for Phase-B queries (within Phase-B block of combined index)
    let qb_ranges: Vec<(usize, usize)> = (0..cfg.clusters)
        .flat_map(|c| {
    ...
    let (fp_ma, fp_p50a, fp_p95a, fp_prec_a) =
        measure_search_cp(&fp, &queries_a, &qa_ranges, cfg.k);
    let (spq_ma, spq_p50a, spq_p95a, spq_prec_a) =
        measure_search_cp(&spq, &queries_a, &qa_ranges, cfg.k);
    let (str_ma, str_p50a, str_p95a, str_prec_a) =
        measure_search_cp(&strpq, &queries_a, &qa_ranges, cfg.k);
    ...
    let (fp_mb, fp_p50b, fp_p95b, fp_prec_b) =
        measure_search_cp(&fp, &queries_b, &qb_ranges, cfg.k);
    let (spq_mb, spq_p50b, spq_p95b, spq_prec_b) =
        measure_search_cp(&spq, &queries_b, &qb_ranges, cfg.k);
    let (str_mb, str_p50b, str_p95b, str_prec_b) =
        measure_search_cp(&strpq, &queries_b, &qb_ranges, cfg.k);
    ...
    // ── output ────────────────────────────────────────────────────────────────
    let fp_qps_a = if fp_ma > 0.0 {
        (1_000_000.0 / fp_ma) as usize
    } else {
        0
    };
    let spq_qps_a = if spq_ma > 0.0 {
        (1_000_000.0 / spq_ma) as usize
    } else {
        0
    };
    let str_qps_a = if str_ma > 0.0 {
        (1_000_000.0 / str_ma) as usize
    } else {
        0
    };
    let fp_qps_b = if fp_mb > 0.0 {
        (1_000_000.0 / fp_mb) as usize
    } else {
        0
    };
    let spq_qps_b = if spq_mb > 0.0 {
        (1_000_000.0 / spq_mb) as usize
    } else {
        0
    };
    let str_qps_b = if str_mb > 0.0 {
        (1_000_000.0 / str_mb) as usize
    } else {
        0
    };
    ...
    print_row(SearchRow {
        ...
        p95: str_p95a,
        qps: str_qps_a,
        mem_kb: strpq.memory_bytes() / 1024,
        prec: str_prec_a,
    });
    ...
    println!(
        "  (StreamPQ periodic retrain overhead: {:.0}× vs StaticPQ)",
    ...
    for c in 0..cfg.clusters {
        ...
        println!(
            "  {:>7}   {:>8.4}   {:>8.4}   {:>+7.4}",
            c, cluster_prec_spq[c], cluster_prec_str[c], delta
        );
    }
    ...
    println!(
        "  [1] FullPrecision Phase-B cluster precision ≥ 0.90 : {fp_prec_b:.4}  → {}",
        if fp_ok { "PASS" } else { "FAIL" }
    );
    println!(
        "  [2] StreamPQ Phase-A cluster precision ≥ 0.60      : {str_prec_a:.4}  → {}",
        if stream_a_ok { "PASS" } else { "FAIL" }
    );
    println!(
        "  [3] StreamPQ Phase-B cluster precision ≥ 0.50      : {str_prec_b:.4}  → {}",

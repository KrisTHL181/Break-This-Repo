...
fn a_content_change_between_runs_is_not_warned_about() {
    ...
    let warm = common::run_fallow_in_root("dead-code", root, &["--quiet"]);
    ...
    .unwrap();
    let after_edit = common::run_fallow_in_root("dead-code", root, &["--quiet"]);
    ...
    .unwrap();
    let perf = common::run_fallow_in_root("dead-code", root, &["--performance"]);
    assert!(
        perf.stderr

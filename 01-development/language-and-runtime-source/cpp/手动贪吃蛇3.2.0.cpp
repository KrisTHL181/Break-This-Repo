...
int main(){
	...
	cout<<"特殊："<<endl;
	cout<<"c：炫彩(8*8)"<<endl;
	cout<<"z：人工智障(8*8)"<<endl;
	cout<<"i：无限模式(20*20)"<<endl;
	cout<<endl;
	cout<<"按q退出"<<endl;
	while(c!='a'&&c!='e'&&c!='m'&&c!='h'&&c!='c'&&c!='z'&&c!='i'&&c!='s'&&c!='q')c=getch();
	if(c=='q')goto start;
	else if(c=='a'){
		N=4;
		mde=0;
	...
	system("cls"); 
	p=N*20;
	if(spe_mode==4||spe_mode==5)p=5000;
	length=1;
	...
	while(1){
		...
		if(spe_mode==4){
			...
			while(1){
				...
				rnc=rand()%4;
				if(rnc==0){
					if(g[h.x][h.y-1]==1||h.y-1<1||length!=1&&(h.y+N-2)%N+1==lasty)continue;
					c=72;
				}
				else if(rnc==1){
					if(g[h.x][h.y+1]==1||h.y+1>N||length!=1&&h.y%N+1==lasty)continue;
					c=80;
				}
				else if(rnc==2){
					if(g[h.x-1][h.y]==1||h.x-1<1||length!=1&&(h.x+N-2)%N+1==lastx)continue;
					c=75;
				}
				else if(rnc==3){
					if(g[h.x+1][h.y]==1||h.x+1>N||length!=1&&h.x%N+1==lastx)continue;
					c=77;
				}
				break;
			}
		...
		if(g[h.x][h.y]==0||dq.front().x==h.x&&dq.front().y==h.y){
			...
			if(length==N*N-1){
				...
				if(autosave)saveg();
				gotoxy(0,0);
				cout<<"你赢了！！！按q返回主界面，按r再玩一次，按e退出"<<endl;
				cout<<"分数:"<<p<<"    "<<endl;
	...
	if(autosave)saveg();
	gotoxy(0,0);
	cout<<"你输了！按q返回主界面，按r再玩一次，按e退出"<<endl;
	cout<<"分数:"<<p<<"    "<<endl;
	cout<<"时间:"<<time(0)-tm<<endl;
	cout<<"长度:"<<length<<endl;
	c='1';
...
bool sure(string stri){
	system("cls"); 
	cout<<"确定"<<stri<<"吗？输入 SuRe 来确定（注意大小写）";
	string suresuresure;
	cin>>suresuresure;

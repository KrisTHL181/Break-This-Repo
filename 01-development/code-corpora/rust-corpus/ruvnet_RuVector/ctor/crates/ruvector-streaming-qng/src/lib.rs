...
mod tests {
    ...
    fn gen_block(n_per_cluster: usize, cfg: &DatasetConfig, shift: f32) -> Vec<Vec<f32>> {
        ...
        for c in 0..cfg.clusters {
            for i in 0..n_per_cluster {
                let centroid: Vec<f32> = (0..cfg.dims)
                    .map(|d| {
                        let base = if d == 0 {
                            c as f32 * 4.0 + shift
                        } else {
    ...
    fn static_pq_cluster_precision_above_floor() {
        ...
        assert!(
            mean_prec >= 0.80,
            "StaticPQ cluster precision={mean_prec:.4} < 0.80"
        );
    }
    ...
    fn stream_pq_adapts_after_distribution_shift() {
        ...
        for (qi, q) in queries_b.iter().enumerate() {
            ...
            let correct_spq = res_spq
                .iter()
                .filter(|h| h.id >= b_start && h.id < b_end)
                .count();
            let correct_str = res_str
                .iter()
                .filter(|h| h.id >= b_start && h.id < b_end)
                .count();
    ...
    #[test]
    fn rebuilding_replaces_existing_vectors() {
        let cfg = make_cfg();
        let first = gen_block(25, &cfg, 0.0);
        let second = gen_block(5, &cfg, cfg.shift);
        ...
        assert_eq!(fp.len(), second.len());
        assert_eq!(spq.len(), second.len());
        assert_eq!(strpq.len(), second.len());
    }
}

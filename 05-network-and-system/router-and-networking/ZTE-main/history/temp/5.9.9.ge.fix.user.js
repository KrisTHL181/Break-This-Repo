...
(function() {
    ...
    const CONFIG = {
        ...
        forceMeshMode: 1, // 【Mesh探测模式】0: 官方拓扑驱动 | 1: 17秒智能等待(默认) | 2: 强制大包抓取(专治阉割、不出数据)
    ...
    addEventListener('load',function(){try{if(this.responseType===''||
        ...
        {window.startGegePrecisionEngine();}}}}catch(e){console.warn("[哥哥科技] XHR 拦截器异常:",e.message);}});
                                             oOp.apply(this,arguments);};function s2b(s){let m=s?.match(/([\d.]+)\s*(G|M|K)?bps/i);if(!m)return 0;let v=parseFloat(m[1]),u=(m[2]||"").toUpperCase();return u==='G'?v*1e9:u==='M'?v*1e6:u==='K'?v*1e3:v;}
									function fB(b){return b>=1e9?`${(b/1e9).toFixed(3)} Gbps`:b>=1e6?`${(b/1e6).toFixed(2)} Mbps`:b>=1e3?`${(b/1e3).toFixed(1)} Kbps`:`${Math.round(b)} bps`;}function fBy(b){return b>=8589934592?`${(b/8589934592).toFixed(3)} GiB/s`:b>=8388608?`${(b/8388608).toFixed(2)} MiB/s`:b>=8192?`${(b/8192).toFixed(1)} KiB/s`:`${Math.round(b/8)} B/s`;}function fV(b){return b>=8796093022208?`${(b/8796093022208).toFixed(4)} TiB`:b>=8589934592?`${(b/8589934592).toFixed(3)} GiB`:b>=8388608?`${(b/8388608).toFixed(2)} MiB`:b>=8192?`${(b/8192).toFixed(1)} KiB`:`${Math.round(b/8)} B`;}function fVD(i,o){return i>=8796093022208?`${(o/8796093022208).toFixed(4)} | ${(i/8796093022208).toFixed(4)} TiB`:i>=8589934592?`${(o/8589934592).toFixed(4)} | ${(i/8589934592).toFixed(3)} GiB`:i>=8388608?`${(o/8388608).toFixed(3)} | ${(i/8388608).toFixed(2)} MiB`:i>=8192?`${(o/8192).toFixed(1)} | ${(i/8192).toFixed(2)} KiB`:`${Math.round(o/8)} | ${Math.round(i/8)} B`;}
									function fSV(b){return(b/=8)>=1073741824?(b/1073741824).toFixed(3)+'G':b>=1048576?(b/1048576).toFixed(2)+'M':b>=1024?(b/1024).toFixed(1)+'K':Math.round(b)+'B';}function fOT(s){if(s<=0)return"";let d=Math.floor(s/86400),h=Math.floor((s%86400)/3600),m=Math.floor((s%3600)/60),sec=Math.floor(s%60);return d>0?`${d}天${h}时${m}分${sec}秒`:`${h}小时${m}分${sec}秒`;}function nM(m){return m?m.toLowerCase().replace(/-/g,':').replace(/\s/g,''):'';}function pI(n){let o=Object.create(null),c=n.children;for(let i=0;i<c.length;i++){if(c[i].tagName==="ParaName"){let k=c[i].textContent,v="",j=i+1;while(j<c.length&&c[j].tagName!=="ParaName"){if(c[j].tagName==="ParaValue"){v=c[j].textContent;i=j;break;}j++;}o[k]=v;}}return o;}const st=document.createElement('style');
    ...
    appendChild(st);window.gegeRenderedMacs=new Set();async function rSD(pWT=null,sT=null){if(
        ...
        cM);window.gegeForceUIRedraw=!1;}let gDt=(S.lt!==0)?(n-S.lt)/1000:0;if(S.wLT===undefined){S.wLT=n;}else if(cWU!==S.wInstUp||cWD!==S.wInstDn){let wDt=n-S.wLT;if(S.wInstUp>0)S.wTotUp+=(S.wInstUp+cWU)*wDt/2000;if(S.wInstDn>0)S.wTotDn+=(S.wInstDn+cWD)*wDt/2000;S.wLT=n;}for(const[m,cC] of Object.entries(cI)){S.cls[m]??={upR:cC.upRate,dnR:cC.dnRate,lUT:n,intUp:0,intDn:0,uB:CONFIG.readSaveData===1?0:cC.offUp,dB:CONFIG.readSaveData===1?0:cC.offDn,lU:cC.offUp,lD:cC.offDn,aR:!1,dpU:0,dpD:0,oU:CONFIG.readSaveData===1?cC.offUp:0,oD:CONFIG.readSaveData===1?cC.offDn:0};
		let cS=S.cls[m],dU=cC.offUp-cS.lU,dD=cC.offDn-cS.lD;if(dU<0||dD<0){if(dU<0){cS.uB+=dU;cS.dpU=cS.lU;}if(dD<0){cS.dB+=dD;cS.dpD=cS.lD;}cS.aR=!0;}else if(cS.aR){if(dD>2516582400||dU>671088640||(cS.dpD&&dD>=cS.dpD)||(cS.dpU&&dU>=cS.dpU)){cS.uB+=dU;cS.dB+=dD;cS.aR=!1;cS.dpU=0;cS.dpD=0;}}if(cS.lOS!==cC.onSec){cS.onS=cC.onSec;cS.lOS=cC.onSec;}else{cS.onS=(cS.onS||cC.onSec||0)+gDt;}if(cC.upRate!==cS.upR||cC.dnRate!==cS.dnR){let ms=n-cS.lUT;if(cS.upR>0)cS.intUp+=(cS.upR+cC.upRate)*ms/2000;else if(cC.upRate>0)cS.intUp+=(cC.upRate/2)*CONFIG.lanRefreshInterval;if(cS.dnR>0)cS.intDn+=(cS.dnR+cC.dnRate)*ms/2000;else if(cC.dnRate>0)cS.intDn+=(cC.dnRate/2)*CONFIG.lanRefreshInterval;cS.upR=cC.upRate;cS.dnR=cC.dnRate;cS.lUT=n;}cS.lU=cC.offUp;cS.lD=cC.offDn;}S.lt=n;S.wInstUp=cWU;S.wInstDn=cWD;rUI(cWU,cWD,cSU,cSD,cI);}catch(e){console.error("[哥哥科技] 周期采样中断:",e);}finally{isF=!1;}}
function rUI(wU,wD,sU,sD,cI){
let tOD=0,LUp=0,LDn=0,hpU=0,hpD=0,abU=0,abD=0,curHpU=0,curHpD=0,cln={};
for(const[k,s] of Object.entries(S.cls)){
    let cC=cI[k],cU=Math.max(0,(s.lU||0)-(s.uB||0)-(s.oU||0)),cD=Math.max(0,(s.lD||0)-(s.dB||0)-(s.oD||0));
    LUp+=s.intUp||0;LDn+=s.intDn||0;hpU+=cU;hpD+=cD;
    if(cC){curHpU+=cU;curHpD+=cD;tOD+=cC.offDn||0;abU+=cC.offUp||0;abD+=cC.offDn||0;}
    cln[k]={up:cU,down:cD,integral_up:s.intUp||0,integral_down:s.intDn||0,status:s.aR?"off":(CONFIG.portMap[cC?.iface]||cC?.iface||"未知接口"),name:cC?.name||k,ip:cC?.ip||"",raw_up:cC?.offUp||0,raw_down:cC?.offDn||0};}
...
S.rTick=((S.rTick||0)+1)&31;if(S.rTick===1||!S.cRT){const cSR=(W,I,H)=>{if(W===0)return 1.0;let M=Math.max(I,H),m=Math.min(I,H),G=Math.abs(I-H);if(I>=0.84*W&&H>=0.75*W&&(M<=1.5*W||G<=0.6*W))return((I+H)/2)/W;if(m<W&&W<=M&&M<=1.5*W)return M/W;return(Math.abs(I-W)<Math.abs(H-W)?I:H)/W;};let rU=cSR(S.wTotUp,LUp,hpU),rD=cSR(S.wTotDn,LDn,hpD),mR=Math.max(rU,rD),rC=mR>1.5?'#ff4c00':mR>1.15?'#FF9800':'#4CAF50';S.cRT=`<span style="color:${rC};font-weight:bold;">${(rU*100).toFixed(2)}%，${(rD*100).toFixed(2)}%</span>`;}
let bd=document.getElementById('zte-geek-board');
if(!bd){bd=document.createElement('div');bd.id='zte-geek-board';let lH='';
if(CONFIG.uiLayout===1)lH=`<div class="geek-row"><span class="geek-label">WAN口速率</span><div class="geek-val-box"><span class="c-up geek-fixed-width" id="gb-wan-up-bytes"></span><span class="c-down geek-fixed-width" id="gb-wan-down-bytes"></span></div><div class="geek-right-box"><span class="c-up" id="gb-wan-up-bps"></span> | <span class="c-down" id="gb-wan-down-bps"></span></div></div><div class="geek-row"><span class="geek-label">局域网代数和</span><div class="geek-val-box"><span class="c-up geek-fixed-width" id="gb-lan-up-bytes">
...
else if(CONFIG.uiLayout===2)lH=`<div class="geek-row"><span class="geek-label">WAN口速率</span><div class="geek-val-box"><span class="c-up geek-fixed-width" id="gb-wan-up-bytes"></span><span class="c-down geek-fixed-width" id="gb-wan-down-bytes"></span></div><div class="geek-right-box"><span class="c-up" id="gb-wan-up-bps"></span> | <span class="c-down" id="gb-wan-down-bps"></span></div></div><div class="geek-row"><span class="geek-label">局域网代数和</span><div class="geek-val-box"><span class="c-up geek-fixed-width" id="gb-lan-up-bytes"></span><span class="c-down geek-fixed-width" id="gb-lan-down-bytes"></span></div><div class="geek-right-box">实时占比：<span class="c-up" id="gb-perc-up"></span> | <span class="c-down" id="gb-perc-down"></span></div></div><div class="geek-row">
...
if(bd.querySelector('#gb-ratio-display')){bd.querySelector('#gb-cur-up-vol').textContent=`🔼 ${fV(curHpU)}`;bd.querySelector('#gb-cur-down-vol').textContent=`🔽 ${fV(curHpD)}`;bd.querySelector('#gb-ratio-display').innerHTML=S.cRT;}
}
for(let m in cI){
let it=oDC[m];
if(!it)continue;
...
let pu=sU>0?(cC.upRate/sU*100):0,pd=sD>0?(cC.dnRate/sD*100):0,bU=enh.querySelector('.zte-bar-up'),bD=enh.querySelector('.zte-bar-down');
    bU.style.setProperty('--p-up',Math.min(pu,100)+'%');bU.querySelector('.v-val').textContent=`🔼 ${fBy(cC.upRate)}`;bU.querySelector('.v-pct').textContent=(wU>0?pu.toFixed(1):0.0)+'%';bD.style.setProperty('--p-down',Math.min(pd,100)+'%');bD.querySelector('.v-val').textContent=`🔽 ${fBy(cC.dnRate)}`;bD.querySelector('.v-pct').textContent=(wD>0?pd.toFixed(1):0.0)+'%';}}});}async function bVD(ol,cX){try{
let mB=(window.gegeHiddenDevices&&Object.keys(window.gegeHiddenDevices).length>0)?'<span style="color: #ff4c00; font-size: 13px; font-weight: normal; margin-left: 10px; font-family: Consolas;">(哥哥科技：智能Mesh适配)</span>':'',h2=[],h52=[],h58=[],hW=[];(cX?.querySelectorAll("OBJ_CLIENTS_ID Instance")||[]).forEach(i=>{let d=pI(i);if(!d.MACAddress)return;let m=nM(d.MACAddress),ip=escapeHTML(d.IPAddress||''),nm=escapeHTML(d.AliasName||d.HostName||'未知设备'),ifc=d.Interface||'',oS=parseInt(d.OnlineDuration||d.OnlineTime||d.LeaseTime||0),tS=fOT(oS),tH=`<div class="gege-online-time" style="color: #999; font-size: 12px; font-family: Consolas; margin-top: 4px;">${tS?'在线：'+tS:''}</div>`,htm=`<div class="col-md-12 col-xs-12 config-item gege-list-item" data-gege-mac="${m}"><div class="config-item-box" style="display: flex; align-items: stretch;"><div class="col-md-5 col-xs-7 logo" style="width: 33%; display: flex; flex-direction: row; align-items: center;"><div class="dev-logo" style="width: 50px; height: 50px; min-width: 50px; margin-right: 15px; background: url('/jquery/static/img/home/unknown_computer.png') 0% 0% / 50px no-repeat; display: inline-block;"></div><div class="dev-intro" style="flex: 1; display: flex; flex-direction: column; justify-content: flex-start; min-height: 100px;">
<div class="dev-name" style="font-weight: bold; color: #333; font-size: 14px;">${nm}</div>${tH}</div></div><div class="col-md-4 col-xs-5 info" style="width: 27%; display: flex; flex-direction: column; padding: 0 10px; border-right: 1px solid #eee;"><div class="dev-ip" style="color: #666; font-family: Consolas;">${ip}</div><div class="dev-number grey" style="color: #999; font-size: 12px; font-family: Consolas;">MAC：${m}</div></div><div class="col-md-3 col-xs-12 speed" style="width: 40%; display: flex; flex-direction: column; justify-content: center; padding: 0 10px;"></div></div></div>`;if(['wl0','wlan0','wlan1','wl1'].includes(ifc))h2.push(htm);else if(['wlan5','wl4','wlan4','wl3','wlan3'].includes(ifc))h52.push(htm);else if(ifc==='wl2'||ifc==='wlan2'||ifc==='wl5'||(/w/i.test(ifc)&&!/wan/i.test(ifc)))h58.push(htm);else hW.push(htm);});requestAnimationFrame(()=>{ol.innerHTML=`<div style="padding: 20px; max-width: 1200px; margin: 0 auto; min-height: 100%;"><div id="gege-board-anchor"></div><div id="config-list" class="config-list gege-list-container"><div class="gege-section"><div class="config-title">无线设备（2.4GHz）</div>${h2.join('')||'<div class="gege-empty-state">没有连接设备</div>'}</div><div class="gege-section"><div class="config-title">无线设备（5.2GHz）</div>${h52.join('')||'<div class="gege-empty-state">没有连接设备</div>'}
...
${hW.join('')||'<div class="gege-empty-state">没有连接设备</div>'}</div></div></div>`;});}catch(e){requestAnimationFrame(()=>{ol.innerHTML=`<div style="padding: 20px; color: red;">数据渲染失败: ${escapeHTML(e.message)}</div>`;});}}window.createGegeFloatingBtn=function(){if(document.getElementById('gege-floating-btn'))return;let b=
    ...
    gegeBActivated){window.gegeBActivated=!0;clearTimeout(window.gegeMasterTimer);window.gegeMasterTimer=setInterval(
        eBET,1000);if(CONFIG.forceMeshMode===2){window.triggerGegeMeshSniper();}else if(CONFIG.forceMeshMode===1){setTimeout(()=>{if(window.gegeLastDevCount<=0){console.log("⏱️ [哥哥科技] 17秒熔断判定生效！目标无响应，强制切入B2模式");CONFIG.forceMeshMode=2;window.triggerGegeMeshSniper();}},17000);}}bVD(o,lCxt?pr.parseFromString(lCxt,"text/xml"):null).then(()=>{if(window.gegeBActivated)eBET();else rSD();});};function iGM(){let mC=document.querySelector('.menu_items');if(!mC)return;let oD=mC.querySelector(
    ...
    Object.keys(aM).length,hI=(m<<2)-10;return Object.keys(aM)[hI^3];};tS.textContent=pT(aM[gHP()],tS.textContent);}lT.querySelectorAll(
        ...
        gegeTogglePanel(!1);}},!0);}window.gegeBActivated=!1;window.gegeEngineRunning=!1;window.gegeLastDevCount=-1;window.gegeLastMeshDevCount=-1;window.gegeHiddenDevices={};window.gegeTimerStarted=!1;window.gegeSyncAnchor=0;window.gegeTickCount=0;window.gegeMasterTimer=null;
        window.triggerGegeMeshSniper=async function(){try{const liR=await fetch(`/?_type=vueData&_tag=localnet_lan_info_lua&_=${Date.now()}`),liX=pr.parseFromString(await liR.text(),"text/xml");let nHD={};liX.querySelectorAll("OBJ_LAN_INFO_ID Instance").forEach(inst=>{let d=pI(inst);if(d.DevMeshType==='3'&&d.Active==='1'&&d.MACAddress){let m=nM(d.MACAddress),bN=d.DevName||d.HostName||d.DisplayedPictureName||d.AliasName||"Mesh设备",bI=d.Interface||"";if(d.IFAliasName==='SSID1')bI='wl0';else if(d.IFAliasName==='SSID5')bI='wl4';nHD[m]={name:bN,iface:bI,origMac:d.MACAddress};}});if(Object.keys(nHD).length>0){window.gegeHiddenDevices=nHD;window.gegeForceUIRedraw=!0;console.log("🎯 [哥哥科技] 破甲弹命中！强制狙击名单:",Object.keys(nHD));}}catch(e){console.warn("[哥哥科技] B2强启拉取失败:",e.message);}};
        ...
        gegeEngineRunning=!0;try{const ts=Date.now(),wR=await fetch(`/?_type=vueData&_tag=vue_home_device_data_no_update_sess&IF_OP=refresh&_=${ts}`);if(
            !wR.ok)throw new Error();const wT=await wR.text(),wST=performance.now();if(!wT.includes('<OBJ_HOME_BASICINFO_ID>'))throw new Error();const wX=
              pr.parseFromString(wT,"text/xml");let bIN=wX.querySelector("OBJ_HOME_BASICINFO_ID Instance"),wI=bIN?pI(bIN):{},cDC=parseInt(
                  wI.AccessDevNum);cDC=isNaN(cDC)?-1:cDC;const lR=await fetch(`/?_type=vueData&_tag=vue_client_data&_=${ts}`);let lT=await lR.text(),lF="";if(lT.includes('<OBJ_CLIENTS_ID>')){let mM=lT.match(/<ParaName>MACAddress<\/ParaName><ParaValue>([^<]+)<\/ParaValue>/g)||[];lF=mM.map(m=>m.replace(/<[^>]+>/g,'')).sort().join('|');}if(cDC!==window.gegeLastDevCount||lF!==window.gegeLastLanFingerprint){if(CONFIG.forceMeshMode===2)await window.triggerGegeMeshSniper();const tR=await fetch(
                  `/?_type=vueData&_tag=vue_topo_data&_=${ts}`);if(!tR.ok)throw new Error();let tJ;
try{tJ=JSON.parse(await tR.text());}catch(e){console.warn("[哥哥科技] 拓扑接口异常，启动静默容错:",e.message);tJ={};}let mDC=tJ.agentlay1?.reduce((s,a)=>s+(parseInt(a.accdevCount)||0),0)||0;if(mDC!==window.gegeLastMeshDevCount){window.gegeLastMeshDevCount=mDC;window.gegeForceUIRedraw=!0;
if(mDC>0&&CONFIG.forceMeshMode!==2){await window.triggerGegeMeshSniper();}else if(mDC===0&&CONFIG.forceMeshMode!==2){window.gegeLastMeshDevCount=0;window.gegeHiddenDevices={};}}
                  window.gegeLastDevCount=cDC;window.gegeLastLanFingerprint=lF;}let hM=Object.keys(

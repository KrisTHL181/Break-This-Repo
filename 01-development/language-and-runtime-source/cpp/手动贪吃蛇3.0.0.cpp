...
int main(){
	...
	gotoxy(3,3);
	cout<<"规则："<<endl;
	cout<<"1、按wasd或上下左右移动，按q退出，按e瞬间退出，按r重来，按空格暂停；"<<endl;
	...
	gotoxy(0,25);
	cout<<"注意：游戏中途退出，记录不会被保存";
	gotoxy(0,27);
	cout<<"代码长度：364行";
	gotoxy(0,0);
	char c='m';
	while(c!='s'&&c!='v'&&c!='q'&&c!=72&&c!=80)c=getch();
	if(c=='q')return 0;
	else if(c==72){
		cl++;
		cl%=4;
	...
	while(1){
		...
		char c=getch();
		if(c=='w'||c==72){
			if(l!=1&&(h.y+N-2)%N+1==lasty)continue;
			h.y--;
		}
		else if(c=='s'||c==80){
			if(l!=1&&h.y%N+1==lasty)continue;
			h.y++;
		}
		else if(c=='a'||c==75){
			if(l!=1&&(h.x+N-2)%N+1==lastx)continue;
			h.x--;
		}
		else if(c=='d'||c==77){
			if(l!=1&&h.x%N+1==lastx)continue;
			h.x++;
		}
